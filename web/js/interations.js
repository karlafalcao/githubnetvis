...
function buildPainel() {
  ...
  svgGroup
    ...
    .append("xhtml:div")
    .html(
      "<h3 style='text-align: center;'>Github Followers Complex Network of Developers by city" +
        `
      </br>
      ...
      ` +
        ...
        "<li>\b Zoom o nó (usuário) na rede ou libere-o quando quiser (use <u>clique+Ctrl</u>)</li>" +
  ...
  svgGroup
    ...
    .attr(
      "transform",
      "translate(" + margin.left + "," + (ht_height + margin.top) + ")",
    )
    // .attr("x", "-14px")
    // .attr("y", "-8px")
    .attr("width", ht_width)
...
function heatmap(user) {
  ...
  let linguagens = d3.keys(ling_repo).sort();
  let xScales = {};

  linguagens.forEach((d) => {
    let temp = d3.scaleBand().domain(ling_repo[d]).range([0, ht_width]); // users: mairatma(outline) henvic
    xScales[d] = temp;
  });

  let yScale = d3.scaleBand().domain(linguagens).range([0, ht_height]);
  ...
  let heatMap = svgGroup
    .append("g")
    .attr("class", "heatmap")
    .attr("width", ht_width);

  // rejoin data
  let rows = heatMap.selectAll(".row").data(linguagens);

  // remove unneeded visualizations
  rows.exit().remove();

  // create new visualizations
  rows.enter().merge(rows).append("g").attr("class", "row");
  ...
  heatMap
    ...
    .attr("width", function (d) {
      return xScales[d[1]].bandwidth();
    })
    .attr("height", function (d) {
      return yScale.bandwidth();
    })
    .attr("x", function (d) {
      return xScales[d[1]](d[0]);
    })
    .attr("y", function (d) {
      return yScale(d[1]);
    })
  ...
  let labels = svgGroup.selectAll(".myText").data(linguagens);

  labels.exit().remove();

  labelsEnter = labels.enter().append("g").attr("class", "myText");
